...
export async function getPricingData() {
    ...
    try {
        // Use cache: 'no-store' combined with tags for on-demand revalidation
        // This ensures fresh data after revalidateTag('pricing') is called
        const response = await fetch(url, {
            next: { tags: ['pricing'] },
            cache: 'no-store' // Disable automatic caching, rely on revalidation
        });
